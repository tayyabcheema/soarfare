...
const TravelerDetailsForm: React.FC<TravelerDetailsFormProps> = ({ onBack, onSubmit }) => {
  const [formData, setFormData] = useState<TravelerFormData>(() => {
    ...
    const flightDetails = sessionStorage.getItem('selectedFlight');
    const passengers = flightDetails ? JSON.parse(flightDetails).passengers : { adults: 1, children: 0, infants: 0 };

    // Create initial arrays for each traveler type based on passenger count
    const adults = Array(passengers.adults).fill(null).map(() => ({ ...emptyTravelerDetails }));
    const children = Array(passengers.children).fill(null).map(() => ({ ...emptyTravelerDetails }));
    const infants = Array(passengers.infants).fill(null).map(() => ({ ...emptyTravelerDetails }));
